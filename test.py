...
class pinhole_process:
    ...
    def project_points(self):
        ...
        img = cv2.rotate(img, cv2.ROTATE_90_COUNTERCLOCKWISE)# for modelX's camera ,rotate the image to fit the correct intrinsic
        ...
        point_cloud1 = o3d.io.read_point_cloud(self.pcd_path + self.file_name + ".pcd")
        points1 = np.array(point_cloud1.points)
        
        point_cloud = copy.copy(point_cloud1)
        point_cloud = point_cloud.transform(self.R_T)

        pc_as_np = np.array(point_cloud.points)
        #o3d.io.write_point_cloud("./pcd_pro/"+self.file_name+".pcd", point_cloud1)
        # vertical degree fov
        vertical = np.arctan2(abs(pc_as_np[:, 1]), pc_as_np[:, 2])
        # horizon degree fov
        horizon = np.arctan2(abs(pc_as_np[:, 0]), pc_as_np[:, 2])
        ...
        for o in range(len(pc_as_np)):
            ...
            #remove outlier
            if u_distorted > img.shape[1] or v_distorted > img.shape[0] or u_distorted < 0 or v_distorted < 0:
                continue
            else:
                selected_point = points1[o]
                new_pcd.points.append(selected_point)
                depth[int(v_distorted)][int(u_distorted)] = pc_as_np[o][2]
        #pcd2.points = o3d.utility.Vector3dVector(new_pcd)
        #o3d.io.write_point_cloud("./pcd_pro/"+self.file_name+".pcd",new_pcd)
    ...
    def visualize(self):
        ...
        #color_intervals = [(0, (0, 0, 255)), (0.2, (0, 255, 255)), (0.4, (0, 255, 0)), (0.6, (255, 255, 0)), (0.8, (255, 165, 0)), (1, (255, 0, 255))]
        ...
        for m_1 in range (count_1):
            for n_1 in range (count_2):
                ...
                if  np.ptp(diffs_masked)>th1:
                    ...
                    #mask[zero_mask1.astype(int)] = False
                    mask[zero_mask] = False
                    occ[x_start : x_end, y_start : y_end][mask] = 0

        #e_o=time.clock()
        #print("remove occlusion time is :",e_o-s_o)
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(0,len(f1),1)):
    ...
    f2.sort()
    f3=os.listdir(rect_pinhole_left_path)
    f3.sort()
    ...
    pool.join()
    s_v=time.time()
    for q in tqdm(range(0,len(f2),1)):
        pinhole=pinhole_process(pinhole_pcd_dir, pinhole_img_left, pinhole_img_right, pinhole_LUT, pinhole_depth_path, pinhole_disparity_path, 
                                pinhole_projection_path, rect_pinhole_left_path, rect_pinhole_right_path, f2[q][0:-4])
        pinhole.load_yaml_file()
        #pinhole.depth_to_disparity()
        pinhole.visualize()
    e_v=time.time()
    print("visualize time is :",e_v-s_v)
    e_p=time.time()
